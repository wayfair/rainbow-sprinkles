...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.11.23") {
        throw new Error(`Cannot start service: Host version "${"0.11.23"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
...
var ensureServiceIsRunning = () => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.11.23"}`, "--ping"), {
    windowsHide: true,
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.11.23"}`), {
    cwd: defaultWD,
    windowsHide: true,
    input: stdin,
